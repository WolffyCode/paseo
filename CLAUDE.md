...
- **All Helm design changes go through `docs/helm/`.** Before any Helm feature/UI change, READ `docs/helm/product.md` and open `docs/helm/ui.html` (the two canonical source-of-truth docs — functions/flows + every screen's full UI & interaction states). After the change, WRITE it back into BOTH and keep them in sync — never update only one. Each requirement also gets a local set under `docs/helm/requirements/<YYYY-MM-DD>-<topic>/` (`requirement.md` WHAT + `ui.html` + `architecture.md` HOW-boundary). Canonical edits land on the mainline so all branches inherit the latest. Full workflow + format spec: [docs/helm/README.md](docs/helm/README.md).
- **Helm features run the multi-agent dev workflow.** Main agent = project manager. Pipeline (3 chairman gates): requirement+UI together → architecture (separate review) → dev (PM picks Codex or internal `helm-developer` agents) → `helm-code-reviewer` → `helm-qa` four-audit → chairman acceptance → sync `develop`. Roles/personalities + skills: [docs/helm/workflow.md](docs/helm/workflow.md). Implementation contract (model/UI separation, no patching, contract-comment per function, mandatory unit tests, requirements are WHAT-not-HOW): [docs/helm/standards.md](docs/helm/standards.md). Reusable agents live in `.claude/agents/helm-*.md`. **Model mandate (hard):** any Paseo `codex` agent MUST be `gpt-5.5` thinking `xhigh` (1M); any `reclaude` agent MUST be `claude-opus-4-8[1m]` (Opus 4.8 1M) thinking `max`. Never lower. Set `--thinking`/`thinkingOptionId` explicitly; defaults live in `~/.paseo/orchestration-preferences.json` (read it before choosing a provider).
