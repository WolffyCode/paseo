...
| [docs/helm/](docs/helm/README.md)                              | **Helm design source-of-truth** — canonical product doc (`product.md`) + UI design (`ui.html`) + per-requirement workflow      |
...
One direction of flow: **feature branch → `develop` → `release`**. This is mandatory.

- **`develop` is the integration trunk — all work starts here.** For every requirement, cut a branch off `develop` (`git checkout develop && git checkout -b <feature>`) and build the change on that branch. Never commit feature work straight onto `develop`.
- **Merge back to `develop` only after the requirement is verified.** "Verified" means the requirement is implemented AND validated — typecheck + lint pass and the feature actually works (not just renders). Unverified or half-finished work never lands on `develop`.
- **`release` is the pre-release (预发布) branch, cut from `develop`.** It is the staging line for daily use. Promote `develop` → `release` (merge or fast-forward) once the integrated work on `develop` is ready to dogfood.
- **All client builds come from `release` — never from a feature branch or from `develop` directly.** Desktop, Android, and iOS builds are produced off `release`, which serves as the daily-use test build. If you need a build, promote to `release` first, then build there.
- **`main` stays the upstream/stable line** (npm publishes, production release tags) and is left untouched by this everyday flow.
...
- **All Helm design changes go through `docs/helm/`.** Before any Helm feature/UI change, READ `docs/helm/product.md` and open `docs/helm/ui.html` (the two canonical source-of-truth docs — functions/flows + every screen's full UI & interaction states). After the change, WRITE it back into BOTH and keep them in sync — never update only one. Each requirement also gets a local set under `docs/helm/requirements/<YYYY-MM-DD>-<topic>/` (`requirement.md` WHAT + `ui.html` + `architecture.md` HOW-boundary). Canonical edits land on the mainline so all branches inherit the latest. **Every requirement triggers a whole-doc self-audit (not just the changed slice):** after writing the local docs and syncing canonical, re-read the ENTIRE `product.md` + `ui.html` and verify (a) **completeness** — no stale/contradictory/missing text, every function described to template, the two canonical docs consistent with each other and with the requirement; (b) **flow connectivity** — trace every flow end-to-end, every state has defined entries/exits, no dead-ends or orphan screens. The develop-guardian PM enforces this on `develop`. Full workflow + format spec: [docs/helm/README.md](docs/helm/README.md).
- **Helm runs a 3-layer org: 总监 → 项目经理 → 员工 (director → PM → employee).** Main agent = **总监 (director)** — owns ONLY the `develop` branch (merges, integrity), commands up to 3 **项目经理 (PMs)**, never does feature work directly. Each **feature PM** owns one module on its own branch off `develop` and schedules its own 5-role team (产品 / UI / 架构 / 开发 / 测试). The **3rd PM is the develop guardian**: after a branch merges, its 产品/UI/测试 regression-validate `develop` against canonical `product.md` / `ui.html` (product / UI / code-quality); discrepancies route back to that team's 开发. Per-project flow with **3 chairman gates**: PM's 产品+UI produce requirement + latest design稿 off develop's canonical → director → **chairman reviews 产品+设计稿 (gate 1)** → PM's 架构 produces the 架构实现方案 (`architecture.md`) → director → **chairman reviews architecture (gate 2)** → 开发 implements + 测试 unit/regression → PM reports green → director → **chairman 验收 (成果) via client/debug (gate 3)** → issues loop back to the team → on chairman OK the director merges to `develop` (team resolves conflicts + reintegrates the main flow) → guardian regression. **Model mandate (hard, 2026-07-08): the main conversation (director) is Claude Fable 5; ALL subagents are launched via Paseo (`mcp__paseo__create_agent`), never via the built-in Agent tool. Code-writing agents use Codex 5.6 Sol (reasoning max); every other role (PM / 产品 / UI / QA / 架构评审) uses Claude Sonnet 5 (thinking max).** **Launch role agents on-demand (role brief attached) — spawn only the one you need, never pre-build all.** Full roles + flow: [docs/helm/workflow.md](docs/helm/workflow.md). Implementation contract (model/UI separation, no patching, contract-comment per function, mandatory unit tests, requirements are WHAT-not-HOW): [docs/helm/standards.md](docs/helm/standards.md).
