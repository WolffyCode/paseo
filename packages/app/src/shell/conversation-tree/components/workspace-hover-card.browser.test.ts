...
import { createHoverSafeZoneTracker, type HoverRect } from "./workspace-hover-card";

const TRIGGER: HoverRect = { left: 0, right: 100, top: 20, bottom: 60 };
const CONTENT: HoverRect = { left: 120, right: 240, top: 20, bottom: 120 };
...
function createHandle(
  rects: { trigger: HoverRect | null; content: HoverRect | null } = {
    trigger: TRIGGER,
    content: CONTENT,
  },
): TrackerHandle {
...
// Equivalent coverage to hooks/hover-safe-zone-tracker.test.ts, against this
// module's inlined copy of the same geometry (architecture §4.2: copy not import).
describe("conversation-tree workspace hover card safe zone", () => {
