...
// deriveContextMenuItems: static ordered id table → label + enabled). Four sets keyed to how the
// content renders: code/text editor (9 items), markdown preview (5), image (4), and the binary/
// unrecognized read-only fallback (3 file actions only — sRS7); markdown-in-edit is a text editor so it
// reuses the editor set. Directory-tree write ops (new/rename/delete/file cut-copy-paste) are
// deliberately in NO set — the file tab never redraws the tree. Items are always enabled: the panel is
// desktop-only, so reveal-in-finder has no capability gate this round.
...
// Binary / unrecognized read-only fallback: no edit or copy-content action, just the file actions shared
// with the image + markdown-preview menus (sRS7).
const READONLY_MENU: ReadonlyArray<ContentMenuItemId> = [
  "copy-file-path",
  "locate-in-tree",
  "reveal-in-finder",
];
...
// (preview → the 5-item read menu; edit → the 9-item editor menu). Binary/unrecognized read-only gets the
// 3 file actions (no edit/copy-content). Live selector — nothing is stored.
